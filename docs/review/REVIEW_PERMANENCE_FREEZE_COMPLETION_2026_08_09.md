...
已经花掉了。因此这是**前置条件**，不是可以事后补跑的校验。

`cal/evaluation/stochastic_permanence_custody.py` 新增
`require_custodian_salt` 与 `open_reserved_split_world`：漏传即 `TypeError`，
传公开盐、过短盐或单字节占位盐即 `InvertibleSplitError`。
放在 custody 是因为它就是一条 custody 义务——该模块其余部分管的是什么可以被
**消费**，这一条管的是什么可以被**生成**。

**这次修订同时是永久性栈修订链的第一次实跑。** custody.py 在 22 个锁定模块内，
所以按 §二 O1 里写明的代价走完了全程：

1. 改动锁定源 → `verify_locked_sources` 拒绝执行；
2. 铸出协议 **V2**，带 `amendment_record`（前身 V1 的路径与 digest、
   `gated_evidence_produced_under_prior_protocol: "none"`、改动理由）——
   `build_permanence_stack_source_lock` 现在**强制 V1 之后的版本必须带修订记录**，
   否则拒绝铸造；
3. **重跑 phase0 与 phase-R 两份产物**，而不是把漂移登记进
   `ACKNOWLEDGED_SOURCE_DRIFT`。

第 3 步值得说明：这段守卫零调用方，**不可能**影响任何已记录的数字，用"声明漂移"
糊过去在技术上说得通。没有那样做，因为那恰恰是本轮要消除的
"锁只是描述性的"状态——第一次遇到代价就绕开，等于没有锁。
